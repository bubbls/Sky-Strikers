Shader "Moon/Other/Default" {
	Properties {
		...
		[Toggle(_AlphaOn)] _Alpha ("Use Alpha Channel", Float) = 0
		_HomeColor ("Home Color", Vector) = (1,1,1,1)
		_AwayColor ("Away Color", Vector) = (1,1,1,1)
		_DefaultColor ("Difault Color", Vector) = (1,1,1,1)
		...
		[Space(20)] [Header(Other Settings)] [Space] [Toggle(_FogOn)] _Fog ("Can Be Affected By Fog", Float) = 0
		[Toggle(_ReceiveShadowOn)] _ReceiveShadow ("Receive Shadow", Float) = 0
		[Toggle(_BakeOn)] _Bake ("Has Baked Light", Float) = 0
		[Toggle(_ForceAOOn)] _ForceAO ("Force Enable AO", Float) = 0
	}
